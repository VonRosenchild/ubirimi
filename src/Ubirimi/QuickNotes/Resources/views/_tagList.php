...
<?php while ($allTags && $tagInList = $allTags->fetch_array(MYSQLI_ASSOC)): ?>
    ...
    <div<?php if (isset($tagId) && $tagInList['id'] == $tagId) echo ' style="background-color: #EEEEEE;"' ?>>
        <span><a href="/quick-notes/tag/<?php echo $tagInList['id'] ?>/<?php if (isset($noteInTag['id'])) echo $noteInTag['id']; else echo '-1'; ?>"><?php echo $tagInList['name'] ?> (<?php echo $tagInList['nr'] ?>)</a></span>
    </div>
<?php endwhile ?>