    ...
    static public function removeDirectory($remove_path) {
        ...
        if (($path = realpath($remove_path)) !== FALSE) {
            if (@chmod($path, 0777) === FALSE) {
                throw new Exception(T_("Can't delete directory %s. Permission denied."), $path);
            }
            try {
                if (is_dir($path)) {
                    $dh = opendir($path);
            ...
            while (($file = readdir($dh)) !== false) {
                if ($file != '.' && $file != '..') {
                    if (is_dir($path . DIRECTORY_SEPARATOR . $file)) {
                        DirectoryUtils::removeDirectory($path . DIRECTORY_SEPARATOR . $file);
                    } else {
            ...
            closedir($dh);
            if (@rmdir($path) === FALSE) {
                throw new Exception(T_("Can't delete directory %s."), $path);
            }
        }
    }
