    ...
    public function getCommentsLayoutHTML($comments, &$htmlLayout, $commentParentId, $identationIndex) {
        ...
        for ($i = 0; $i < count($comments); $i++) {
            ...
            if ($comments[$i]['parent_comment_id'] == $commentParentId && $comments[$i]['printed'] == 0) {
                ...
                $htmlLayout .= '<img src="' . UbirimiContainer::get()['repository']->get('ubirimi.user.user')->getUserAvatarPicture(UbirimiContainer::get()['session']->get('user'), 'small') . '" style="vertical-align: top" />';
                $htmlLayout .= '</td>';
                $htmlLayout .= '<td>';
