...
class Project
{
    ...
    public function renderTreeComponentsInViewIssue($component, $htmlComponent) {
        ...
        $parentComponent = UbirimiContainer::get()['repository']->get('yongo.project.project')->getParentComponent($component['parent_id'], 'array');
        if ($parentComponent) {
            UbirimiContainer::get()['repository']->get('yongo.project.project')->renderTreeComponentsInViewIssue($parentComponent, $htmlComponent);
        } else {
    ...
    public function renderTreeComponentsInCombobox($components, $identationIndex, $arrSelectedIssueComponents = null, &$printedComponents) {
        while ($components && $component = $components->fetch_array(MYSQLI_ASSOC)) {
            ...
            if (!in_array($component['id'], $printedComponents)) {
                ...
                $subComponents = UbirimiContainer::get()['repository']->get('yongo.project.project')->getSubComponents($component['id']);
                $identationIndex++;

                if ($subComponents) {
                    UbirimiContainer::get()['repository']->get('yongo.project.project')->renderTreeComponentsInCombobox($subComponents, $identationIndex, $arrSelectedIssueComponents, $printedComponents);
                }
                $identationIndex--;
            }
        }
    ...
    public function renderTreeComponents($components, $identationIndex) { ?>
        <?php while ($components && $component = $components->fetch_array(MYSQLI_ASSOC)): ?>
            ...
            <?php
                ...
                $identationIndex++;
                if ($subComponents) {
                    UbirimiContainer::get()['repository']->get('yongo.project.project')->renderTreeComponents($subComponents, $identationIndex);
                }
                $identationIndex--;
            ?>
        <?php endwhile ?> <?php
    ...
    public function deleteById($Id) {
        ...
        UbirimiContainer::get()['repository']->get('yongo.project.project')->removeHelpdeskData($Id);
...
//    }
    ...
    public function checkProjectsBelongToClient($clientId, $projectIds) {
        for ($pos = 0; $pos < count($projectIds); $pos++) {
            $projectFilter = UbirimiContainer::get()['repository']->get('yongo.project.project')->getById($projectIds[$pos]);
